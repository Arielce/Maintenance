...
public abstract class AbsNoLazyLoadListFragment extends BaseFragment implements IList {
    ...
    protected final void init() {
        ...
        mLoadMoreWrapper.setOnLoadListener(new LoadMoreWrapper.OnLoadListener() {
            ...
            public void onLoadMore() {
                if (isCanLoadMore)
                    AbsNoLazyLoadListFragment.this.loadMore();
            }
        });
    }
    ...
    protected final void onDataSuccessReceived(int pageIndex, List items) {
        showContent();
        if (pageIndex == getInitPageIndex() && items.size() <= 0) {//刷新无数据
            showEmpty(getEmptyMsg());
        } else if(pageIndex == getInitPageIndex() && items.size() <20){//刷新且有数据,但数据不足20条，说明已经无需加载更多了
            mItems.clear();
            mItems.addAll(items);
            disAbleLoadMore();
        }else if (pageIndex == getInitPageIndex()) {//刷新且有数据
            mItems.clear();
            mItems.addAll(items);
