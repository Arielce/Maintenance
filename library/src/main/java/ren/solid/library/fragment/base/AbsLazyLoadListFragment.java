...
public abstract class AbsLazyLoadListFragment extends LazyLoadFragment implements IList {
    ...
    protected final void init() {
        ...
        mLoadMoreWrapper.setOnLoadListener(new LoadMoreWrapper.OnLoadListener() {
            ...
            public void onLoadMore() {
                if (isCanLoadMore)
                    AbsLazyLoadListFragment.this.loadMore();
            }
        });
    }
    ...
    protected final void onDataSuccessReceived(int pageIndex, List items) {
        ...
        } else if (pageIndex == getInitPageIndex()&&items.size()< 20) {//刷新但是条目数小于20条，即不够一页，那么表明没有更多数据了
            mItems.clear();
            mItems.addAll(items);
            disAbleLoadMore();
